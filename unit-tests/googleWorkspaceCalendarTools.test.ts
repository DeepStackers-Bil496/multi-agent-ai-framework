...
describe("GoogleWorkspaceAgent calendar tools", () => {
  ...
  it("creates an event", async () => {
    global.fetch = async (input, init) => {
      ...
      if (url === `${CALENDAR_API_BASE}/calendars/primary/events`) {
        ...
        return buildResponse({
          status: 200,
          body: { id: "event-1", summary: "Standup" },
        });
      }
    ...
    expect(result).toMatch(/Event created/);
    expect(result).toMatch(/event-1/);
  });
  ...
  it("lists upcoming events", async () => {
    ...
    expect(result).toMatch(/Found 1 upcoming event/);
  ...
  it("finds free slots", async () => {
    ...
    expect(result).toMatch(/Available slots/);
  });
});
